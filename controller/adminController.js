...
const getAdminHome = async (req, res) => {
  try {
    ...
    const blockedCategories = await Category.countDocuments({
      isBlocked: true,
    });

    const totalHomeCarousels = await HomeCarousel.countDocuments();
    const blockedHomeCarousels = await HomeCarousel.countDocuments({
      isBlocked: true,
    });

    const totalAdCarousels = await AdCarousel.countDocuments();
    const blockedAdCarousels = await AdCarousel.countDocuments({
      isBlocked: true,
    });
...
const adminAddNewCategory = async (req, res) => {
  try {
    ...
    if (!categoryName) {
      return res
        .status(400)
        .json({ success: false, message: "Data adding error." });
    }
  ...
  try {
    ...
    if (!category) {
      ...
      if (saveCategory) {
        ...
        return res
          .status(500)
          .json({ success: false, message: "Category status changing error." });
      }
    }
  } catch (error) {
    return res.redirect("/admin/adminErrorPage");
...
const updateCategory = async (req, res) => {
  try {
    ...
    if (!categoryId || !categoryName || !categoryDesc) {
      return res
        .status(400)
        .json({ success: false, message: "Form data fetching error." });
    }
    ...
    if (!existingCategory) {
      ...
      if (updateCategory) {
        ...
        return res
          .status(500)
          .json({ success: false, message: "Category updating failed." });
      }
    }
  } catch (error) {
    return res.redirect("/admin/adminErrorPage");
...
const postAdminHomeCarousel = async (req, res) => {
  try {
    ...
    const existingHomeCarousel = await HomeCarousel.findOne({
...
const adminBlockHomeCarousel = async (req, res) => {
  try {
    const homeCarousel = await HomeCarousel.findById({
      _id: req.params.homeCarouselId,
    });
    if (!homeCarousel) {
      return res
        .status(404)
        .json({ success: false, message: "Home carousel not found" });
    } else {
...
const adminDeleteHomeCarousel = async (req, res) => {
  try {
    ...
    if (!homeCarousel) {
      ...
      return res
        .status(200)
        .json({ success: true, message: "Home carousel deleted successfully" });
    }
  } catch (error) {
    return res.redirect("/admin/adminErrorPage");
  }
...
const adminUpdateHomeCarousel = async (req, res) => {
  try {
    const { homeCarouselId, homeCarouselTagline, homeCarouselDescription } =
      req.body;
...
const postAdminAdCarousel = async (req, res) => {
  try {
    ...
    const existingAdCarousel = await AdCarousel.findOne({
      name: adCarouselName,
    });
    if (existingAdCarousel) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Ad Carousel already exists" });
    }
    ...
    if (saveAdCarousel) {
      return res
        .status(200)
        .json({ success: true, message: "Ad Carousel added successfully" });
    } else {
      return res
        .status(400)
        .json({ success: false, message: "AdCarousel adding failed." });
    }
  } catch (error) {
    return res.redirect("/admin/adminErrorPage");
  }
...
const adminBlockAdCarousel = async (req, res) => {
  try {
    if (!req.params.adCarouselId) {
      return res
        .status(400)
        .json({ success: false, message: "Ad carousel ID is required" });
    }

    const adCarousel = await AdCarousel.findById({
      _id: req.params.adCarouselId,
    });

    if (!adCarousel) {
      return res
        .status(404)
        .json({ success: false, message: "Advertisement carousel not found" });
    } else {
...
const adminDeleteAdCarousel = async (req, res) => {
  try {
    ...
    if (!adCarousel) {
      ...
      const deleteAdCarousel = await AdCarousel.findByIdAndDelete(
        req.params.adCarouselId
      );
      if (deleteAdCarousel) {
        const imagePath = path.join(
          __dirname,
