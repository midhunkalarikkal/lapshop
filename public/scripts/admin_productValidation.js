...
const regex = /^[a-zA-Z0-9\s!@#$%^&*()_+\-=~`{}\[\]:;"'<>,.?/\\|]{4,30}$/;
...
graphicsCardError.textContent = 'Graphics card must be between 4 and 30 characters and can include letters, numbers, spaces, and special characters.';
return false;
} else {
graphicsCardInput.classList.remove('is-invalid');
graphicsCardError.textContent = '';
...
const regex = /^[\s\S]{20,250}$/; // Matches any character, between 20 and 250 characters.
...
aboutError.textContent = 'Product about must be between 20 and 250 characters.';
return false;
} else {
aboutInput.classList.remove('is-invalid');
aboutError.textContent = '';
...
const batteryEnergyError = document.getElementById('productBatteryEnergyError');
